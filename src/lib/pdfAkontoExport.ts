...
// ─── Restbetrag-Rechnung (Schlussrechnung) ──────────────────────────────────
export async function exportRestbetragPDF(data: RestbetragExportData) {
  ...
  const rightColCenterX = (pageW / 2 - 4 + pageW) / 2;
  doc.text("SCHLUSSRECHNUNG", rightColCenterX, 26, { align: "center" });
  ...
  // ── Rechnungsempfänger ──────────────────────────────────────────
  const empX = margin; const empY = 76;
  doc.setFillColor(...XLGRAY);
  doc.rect(0, 68, pageW / 2 - 4, 42, "F");
  ...
  autoTable(doc, {
    ...
    styles: { fontSize: 8.5, cellPadding: { top: 4, bottom: 4, left: 3, right: 3 }, textColor: GRAY },
    headStyles: { fillColor: [80, 80, 80] as [number, number, number], textColor: WHITE, fontStyle: "bold", fontSize: 8 },
    columnStyles: { 0: { cellWidth: 10, halign: "center" }, 1: { cellWidth: 55 }, 2: { cellWidth: 30 }, 3: { cellWidth: 12, halign: "center" }, 4: { halign: "right" }, 5: { halign: "right" } },
    alternateRowStyles: { fillColor: XLGRAY },
    tableLineColor: LGRAY,
    tableLineWidth: 0.1,
  });
  ...
  // Akonto-Abzug (in Accent-Farbe)
  doc.setTextColor(...ACCENT);
  doc.text(`Abzüglich Akonto (${data.akontoPercent}%)`, sumX, sumY);
  doc.text(`- ${formatCHF(data.akontoBetrag)}`, pageW - margin, sumY, { align: "right" });
  sumY += 6;
  ...
  // Akzentlinie unter Box
  doc.setFillColor(...ACCENT);
  doc.rect(sumX, totalBoxBottom, sumW, 1.5, "F");

  // ── Zahlungsbedingungen ─────────────────────────────────────────
  const termsY = totalBoxBottom + 10;
  doc.setFont("helvetica", "bold"); doc.setFontSize(8.5); doc.setTextColor(...BLACK);
  doc.text("ZAHLUNGSBEDINGUNGEN", sumX, termsY);
  doc.setFont("helvetica", "normal"); doc.setFontSize(7.5); doc.setTextColor(...GRAY);
  const termsText = data.company.zahlungsbedingungen || "Zahlung fällig innerhalb von 30 Tagen nach Rechnungsdatum.";
  const termsLines = doc.splitTextToSize(termsText, pageW - sumX - margin);
  doc.text(termsLines, sumX, termsY + 6);
  ...
  if (data.company.qr_bill_image_url) {
    const qrImg = await loadImageAsBase64(data.company.qr_bill_image_url);
    if (qrImg) { doc.addPage(); doc.addImage(qrImg, "PNG", 0, 0, doc.internal.pageSize.getWidth(), doc.internal.pageSize.getHeight()); }
  } else {
