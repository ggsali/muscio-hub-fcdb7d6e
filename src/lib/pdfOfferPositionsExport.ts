...
export async function exportOfferPositionsPDF(
  ...
  if (discountPercent > 0) sumRows.push([`Rabatt ${discountPercent}%`, `- CHF ${discountAmount.toFixed(2)}`]);
  ...
  const hinweisText = offerNote || `Dieses Angebot ist gültig bis ${gueltigBis}. Bei Annahme erstellen wir eine verbindliche Auftragsbestätigung.`;
