...
const KundeLogin = () => {
  ...
  useEffect(() => {
    const redirect = async (userId: string) => {
      ...
      else navigate("/mein-konto", { replace: true });
    };
    const { data: sub } = supabase.auth.onAuthStateChange((event, session) => {
      if ((event === "SIGNED_IN" || event === "TOKEN_REFRESHED") && session) redirect(session.user.id);
    });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-muted p-4">
      <div className="bg-card rounded-2xl shadow-lg p-8 w-full max-w-sm border border-border">
        <div className="text-center mb-8">
          <h1 className="font-heading text-2xl font-extrabold text-foreground">
            Willkommen zurück
          </h1>
          <p className="text-muted-foreground text-sm mt-2">Melde dich in deinem Konto an</p>
        </div>
        ...
        <div className="relative mb-4">
          ...
          <div className="relative flex justify-center text-xs text-muted-foreground"><span className="bg-card px-2">oder per E-Mail</span></div>
        </div>
