...
const Email = ({ name, uploadUrl }: Props) => (
  <Html lang="de" dir="ltr">
    <Head />
    <Preview>Dateianforderung</Preview>
    <Body style={{ backgroundColor: '#f4f4f5', fontFamily: 'Inter, Arial, sans-serif', margin: 0, padding: '24px 0' }}>
      <Container style={{ maxWidth: '600px', margin: '0 auto', padding: '0 16px' }}>
        ...
        <Section style={{ backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderTop: 'none', borderRadius: '0 0 12px 12px', padding: '32px 28px' }}>
          <Text style={{ fontSize: '22px', fontWeight: 700, color: '#18181b', margin: '0 0 16px' }}>Dateianforderung</Text>
          <Text style={{ fontSize: '14px', lineHeight: 1.6, color: '#3f3f46', margin: '0 0 14px' }}>
            Guten Tag {name || 'Kunde'},
          </Text>
          <Text style={{ fontSize: '14px', lineHeight: 1.6, color: '#3f3f46', margin: '0 0 14px' }}>
            Bitte laden Sie Ihre 3D-Datei über den untenstehenden Link hoch. Über den Link können Sie die Dateien sicher übertragen – ganz ohne Konto.
          </Text>
...
export const template: TemplateEntry = {
  component: Email,
  subject: 'Dateianforderung',
  displayName: 'Datei-Anforderung',
  previewData: { name: 'Max Muster', uploadUrl: 'https://3dmuscio.com/upload/abc123' },
}
